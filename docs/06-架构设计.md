...
|---|---|---|---|
| `flutter` | sdk | — | — |
| `flutter_localizations` | sdk | Flutter 控件本地化支持（PRD C-17） | `zh_CN` + `en` 界面与教学内容 |
...
| **duel 离线异步对决** | |
| `lib/domain/duel/async_duel_codec.dart` | 挑战码/成绩码编解码、版本与校验和校验、本地唯一解验证、同局成绩比较；挑战码不包含答案 |
| **hint 提示** | |
...
| **teaching 教学** | |
| `lib/domain/teaching/demo_controller.dart` | 原理演示步进：手动/自动播放 2s、上一步、重播、任意步骤跳转；不设置强制观看门槛（P0-EDU-02） |
...
| `lib/ui/features/async_duel/async_duel_page.dart` | 离线对决大厅：发起挑战、导入挑战码、比较成绩码（S-13） |
| `lib/ui/features/async_duel/async_duel_result_dialog.dart` | 完赛成绩与成绩码弹窗，支持复制并返回对决大厅 |
...
| `lib/l10n/{language,app_localizations}.dart` | 简体中文 / English 界面词条、教程渲染、Locale delegate 与回退规则 |
...
| `cover` | 被覆盖的行/列区域 | 天蓝 `#0284C7` | 无边框浅底 | Fish 族 |
