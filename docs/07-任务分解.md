...
| **T-EDU-02** | domain+ui | **原理演示关控制器 + 演示页**（P0-EDU-02、S-03） | T-EDU-01, T-UI-07 | `domain/teaching/demo_controller.dart`、`ui/features/demo/{demo_page,technique_progress_bar,step_control_bar,narration_card}.dart` | 盘面**只读不可填数**；默认手动「下一步」，另有自动播放、上一步、重播；技巧进度条全段可拖动/点击并标出各技巧首次出现节点；允许随时返回或进入下一关；宽屏双栏、窄屏可滚动单栏且控制条可换行 |
